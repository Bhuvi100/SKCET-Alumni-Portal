...
const GetComments = (post_id) => {
    setCom(!com);
    setPostid(post_id);
    axios().get(`http://127.0.0.1:8000/getComment/${post_id}`).then((response) => {
...
{showMod !== false && <Modal data = {pid} setShowModal={setShowMod}/>}
        {
        ...
        <div class="bg-white shadow rounded-lg pb-4 mt-7">
                  <div class="flex flex-row px-2 py-3 mx-3">
                      ...
                      <div class="flex flex-col mb-2 ml-4 mt-1">
                          <div class="flex text-gray-600 text-sm font-semibold"><span class="flex-1 flex-shrink-0">{posts.user.name}</span>
                  ...
                  <div class="text-gray-600 font-semibold  mb-2 mx-3 px-2" >{posts.title}</div>
                  <div class="text-gray-500 text-sm mb-6 mx-3 px-2">{posts.description}</div>
  ...
  <path stroke-linecap="round" stroke-linejoin="round" d="M6.633 10.5c.806 0 1.533-.446 2.031-1.08a9.041 9.041 0 012.861-2.4c.723-.384 1.35-.956 1.653-1.715a4.498 4.498 0 00.322-1.672V3a.75.75 0 01.75-.75A2.25 2.25 0 0116.5 4.5c0 1.152-.26 2.243-.723 3.218-.266.558.107 1.282.725 1.282h3.126c1.026 0 1.945.694 2.054 1.715.045.422.068.85.068 1.285a11.95 11.95 0 01-2.649 7.521c-.388.482-.987.729-1.605.729H13.48c-.483 0-.964-.078-1.423-.23l-3.114-1.04a4.501 4.501 0 00-1.423-.23H5.904M14.25 9h2.25M5.904 18.75c.083.205.173.405.27.602.197.4-.078.898-.523.898h-.908c-.889 0-1.713-.518-1.972-1.368a12 12 0 01-.521-3.507c0-1.553.295-3.036.831-4.398C3.387 10.203 4.167 9.75 5 9.75h1.053c.472 0 .745.556.5.96a8.958 8.958 0 00-1.302 4.665c0 1.194.232 2.333.654 3.375z" />
                  ...
                  <div class="flex w-full border-t border-gray-100">
                      <div class="mt-3 mx-5 flex flex-row text-xs">
                          <button class="flex text-gray-700 font-normal rounded-md mb-2 mr-4 items-center" onClick = {() => GetComments(posts.id)}>Comments:<div class="ml-1 text-gray-400 text-ms"> 30</div></button>
                  ...
                  {com  && posts.id == postid && comment.map((obj) => (
        ...
        <div
            ...
            <div class="flex flex-row " >
                ...
                <div class="flex-col mt-1">
                    <div class="flex items-center flex-1 px-4 font-bold leading-tight">{obj.user.name}
                        <span class="ml-2 text-xs font-normal text-gray-500">2 weeks ago</span>
                    </div>
                    <div class="flex-1 px-2 ml-2 text-sm font-medium leading-loose text-gray-600">{obj.description}
                    </div>
                    <button class="inline-flex items-center px-1 pt-2 ml-1 flex-column" onClick = {() => {setRep(true); setData("comment_id", obj.id); setCommentid(obj.id); console.log(commentid)}}>
            ...
            {obj.replies.map((reply) => (  <div>
                ...
                <div class="flex-col mt-1" >
                    ...
                    <div class="flex-1 px-2 ml-2 text-sm font-medium leading-loose text-gray-600">{reply.description}
                    </div>   
                </div>
              </div>           
            </div>     
            ))}
            {
                ...
                <form class="relative flex items-center self-center w-full max-w-xl p-4 overflow-hidden text-gray-600 focus-within:text-gray-400" onSubmit={handleSubmitCommentsAndReply}>
...
<form onSubmit={handleSubmitCommentsAndReply} class="relative flex items-center self-center w-full max-w-xl p-4 overflow-hidden text-gray-600 focus-within:text-gray-400">
                  <img class="w-10 h-10 object-cover rounded-full shadow mr-2 cursor-pointer" alt="User avatar" src="https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&amp;ixlib=rb-1.2.1&amp;auto=format&amp;fit=crop&amp;w=2000&amp;q=80"/>
                  <span class="absolute inset-y-0 right-0 flex items-center pr-6">
                      <button type="submit" class="p-1 focus:outline-none focus:shadow-none hover:text-blue-500">
                      <svg class="ml-1" viewBox="0 0 24 24" width="16" height="16" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round"><line x1="22" y1="2" x2="11" y2="13"></line><polygon points="22 2 15 22 11 13 2 9 22 2"></polygon></svg>
